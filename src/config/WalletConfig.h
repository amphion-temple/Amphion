...
namespace WalletConfig
{
    ...
    const std::string_view addressPrefix = "amp";

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = "AMP";
    ...
    const std::string daemonName = "Amphiond";

    /* The name to call this wallet */
    const std::string walletName = "Amphion Wallet";

    /* The name of service/walletd, the programmatic rpc interface to a
       wallet */
    const std::string walletdName = "amphion-delphi";

    /* The full name of your crypto */
    const std::string coinName = std::string(CryptoNote::Amphion);

    /* Where can your users contact you for support? E.g. discord */
    const std::string contactLink = "http://Amphion.icu";

    /* The number of decimals your coin has */
    const uint8_t numDecimalPlaces = CryptoNote::parameters::CRYPTONOTE_DISPLAY_DECIMAL_POINT = 2;

    /* The length of a standard address for your coin */
    const uint16_t standardAddressLength = 67;
    ...
    const uint64_t defaultFee = CryptoNote::parameters::MINIMUM_FEE = 10;

    /* The minimum fee value to allow with transactions (in ATOMIC units!) */
    const uint64_t minimumFee = CryptoNote::parameters::MINIMUM_FEE = 10;
